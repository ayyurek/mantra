...
function getFontPath(fontTag: string) {
	...
	return fontEntry ? fontEntry[1] : (console.log("Font not found."), "font-anova");
}

//Make sure the executable for printing has been moved to server-chunks
const addPrintExecutable = () => {
	const targetDirectory = path.join(__dirname, "..", "..", "..", "vendor-chunks");
	const sourceFilePath = path.join("node_modules", "pdf-to-printer", "dist", "SumatraPDF-3.4.6-32.exe");
	const destinationFilePath = path.join(targetDirectory, "SumatraPDF-3.4.6-32.exe");
